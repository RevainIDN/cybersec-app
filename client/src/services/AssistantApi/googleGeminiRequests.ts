...
1. **Анализы**: проверка IP, URL, доменов и файлов на вирусы через API VirusTotal (страница "Анализы").
2. **Уязвимости**: проверка утечек почты и паролей через API HaveIBeenPwned, перевод коротких ссылок в полные (не используя api HaveIBeenPwned) (страница "Уязвимости").
3. **Пароли**: проверка стойкости пароля, генерация новых паролей (страница "Пароли").
4. **Отчёты**: статистика и тренды кибербезопасности (страница "Отчёты").
...
Сервис поддерживает русский, украинский и английский языки.
